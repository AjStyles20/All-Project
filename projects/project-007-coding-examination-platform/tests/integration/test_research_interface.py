...
def setup_run(tmp_path):
    db = Database(tmp_path / "interface.db")
    db.initialize()
    core = P001Repository(db)
    runs = ExperimentRunRepository(db)
    obs = ExperimentObservationRepository(db)
    ...
    runs.create(ExperimentRun(
        ...
        ExperimentRunStatus.ACTIVE, started_at=now,
    ))
...
def test_inspection_exposes_b0_b4_burden_and_independent_reference(tmp_path):
    ...
    obs.add_reference("EXP-I", IndependentReferenceJudgment(
        "ASSESSOR-01", "CC3", EvidenceState.SUPPORTED,
        "Independent judgment.", "AR-v1"
    ), now)
    view = ResearchInterface(runs, obs).inspect_experiment("EXP-I")
